...
NodeInfo* Graph::getNode(int id) const {
    if (id < 0 || id >= modes.size()) {
        cout << "getNode id: "<< id << " does not exist" << endl;
        return nullptr;
    }
    return nodes[id];
}
...
void Graph::updateConnection(int v, int u, double w) {
    ...
    Connection c = Connection(v, u, w);
    adjacencyList[v][u] = c;  // REMEMBER THAT this is indexing into a vector of a map (that takes in a int as key)
}
