...
vector<double> NeuralNetwork::predict(DataInstance instance) {
    ...
    flush();  //just incase we have previous activation values etc etc

    int n = nodes.size();
    vector <int> remainingConnections(n,0); 
     // we need to know how many remaining connections to each node r left before we compute activation function
    for (int v = 0; v < n; v++){
	   for (auto& kv : adjacencyList[v]){  // we add up all the connections that each node in adjacency list has
            int u = kv.first;  // accesses the first part of the key-value pair and in this case the key is the destination node
            remainingConnections(u)++;  // add 1 for each connection found TO THE DESTINATION NODE
        }
    }
    queue <int> q; 
    for (int i = 0; i < inputNodeIds.size(); i++){
        int inID = inputNodeIds[i];   // get the input node id so we can find it in the nodes vector
        NodeInfo inpNode = nodes[inID];
        inpNode->postActivationValue = input[inID]; 
        // SINCE input nodes dont have to do the whole activation summing the weights etc etc process --> make it equal to the input value that we got from instance.x
        remainingConnections[inID] = 0;
        q.push(inID);  //u can just push the inID cause that is all thats needed to access all the other info
    }
    ...
    // for (auto &kv : adjacencyList[v]) {
    //     Connection &c = kv.second;
    //     int u = c.dest;
    //     visitPredictNeighbor(c);
    //     remainingConnections[u]--;
    //     if (remainingConnections[u] == 0) {
    //         q.push(u);
    //     }
    // }
    // }
